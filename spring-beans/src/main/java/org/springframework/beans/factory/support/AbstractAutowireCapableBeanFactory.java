...
public abstract class AbstractAutowireCapableBeanFactory extends AbstractBeanFactory
	...
	protected Object createBean(String beanName, RootBeanDefinition mbd, @Nullable Object[] args)
		 ...
		 * 如果解析的 class 不为空，则会将该 BeanDefinition 进行克隆至 mbdToUse，这样做的主要目的是以为动态解析的 class 是无法保存到共享的 BeanDefinition 中。
		 */
	...
	protected Object doCreateBean(final String beanName, final RootBeanDefinition mbd, final @Nullable Object[] args)
		...
		/***
		 * 1.
		 * 	如果是单例模式，则清除 factoryBeanInstanceCache 缓存，如果存在 ,返回 BeanWrapper 实例对象，。
		 */
		if (mbd.isSingleton()) {
        ...
        */
		...
		try {
			 ...
			 *  init-method、 InitializingBean 、还有 BeanPostProcessor在这边回调
			 */
			//初始化Bean对象
	 ...
	 */
	/**
	 * 如果缓存中没有 BeanWrapper 实例对象或者该 bean 不是 singleton，则调用 createBeanInstance() 创建 bean 实例。
	 * 该方法主要是根据参数 BeanDefinition、args[] 来调用构造函数实例化 bean 对象。过程较为复杂，如下：
	 * @param beanName
	 * @param mbd
	 * @param args
	 * @return
	...
	protected BeanWrapper createBeanInstance(String beanName, RootBeanDefinition mbd, @Nullable Object[] args) {
		...
		/***
		 * 4.如果缓存中没有，则需要先确定到底使用哪个构造函数来完成解析工作，因为一个类有多个构造函数，每个构造函数都有不同的构造参数，
	...
	protected void populateBean(String beanName, RootBeanDefinition mbd, @Nullable BeanWrapper bw) {
		...
		// 不是程序自身定义的 & 持有 InstantiationAwareBeanPostProcessor
		if (!mbd.isSynthetic() && hasInstantiationAwareBeanPostProcessors()) {
